...
const pageContent = <T>(data: T[] | { content?: T[] }) =>
...
const normalizePerson = (person: Entity): Pessoa => ({
  ...
  palestrante: person.palestrante
    ...
    : null,
  organizador: person.organizador
    ? normalizeProfile<Organizador>(person.organizador, {
        instituicao: person.organizador.instituicao || '',
        cargo: person.organizador.cargo || '',
      })
    : null,
  colaborador: person.colaborador
    ? normalizeProfile<Colaborador>(person.colaborador, {
...
const normalizeDiscipline = (discipline: Entity): Disciplina => ({
  ...
  professorId:
    entityId(discipline, 'professor') ||
    toId(discipline.id_professor ?? discipline.professorId),
})
...
const normalizeParticipationEvent = (
  event: Entity | null | undefined
): Participacao['evento'] => {
...
const normalizeParticipationPerson = (
  person: Entity | null | undefined
): Pessoa | null => {
...
const normalizeCertificate = (certificate: Entity): Certificado => ({
  ...
  participacaoId:
    entityId(certificate, 'participacao') || toId(certificate.participacaoId),
  ...
  urlPublica:
    certificate.urlPublica ||
    `/certificados/${certificate.codigoValidacao || ''}`,
  caminhoPdf: certificate.caminhoPdf || '',
  participacao: certificate.participacao
    ? normalizeParticipation(certificate.participacao)
    : null,
})
...
const normalizeProfile = <T extends { pessoaId: string }>(
  item: Entity,
  extra: Omit<T, 'id' | 'pessoaId'>
): T =>
  ({
    id: toId(item.id),
...
class ApiDatabase {
    ...
    if (usuario) {
      ...
      const role =
        normalizeAuthRole(
          tokenRole,
          storedRole,
          typeof parsed === 'object' ? parsed.role : null
        ) || 'USER'
  ...
  async getMyParticipations(): Promise<ParticipacaoUsuarioResponse[]> {
    const response =
      await apiClient.get<ParticipacaoUsuarioResponse[]>('/me/participacoes')
    return response.data
  }

  async getMyCertificates(): Promise<CertificadoUsuarioResponse[]> {
    const response =
      await apiClient.get<CertificadoUsuarioResponse[]>('/me/certificados')
    return response.data
  }

  async getMyMedals(): Promise<MedalhaUsuarioResponse[]> {
    const response =
      await apiClient.get<MedalhaUsuarioResponse[]>('/me/medalhas')
  ...
  async saveDiscipline(discipline: Partial<Disciplina>): Promise<void> {
    const payload = {
      ...
      id_professor: discipline.professorId
        ? Number(discipline.professorId)
        : null,
    }
  ...
  async createPublicInscription(
    eventId: string,
    payload: InscricaoPublicaRequest
  ): Promise<InscricaoPublicaResponse> {
    const response = await apiClient.post(
      `/eventos/${eventId}/inscricoes`,
      payload
    )
    return response.data
  }
  ...
  async saveEvent(
    ...
    const evento = {
      ...
      patrocinadorId: event.patrocinadorId
        ? Number(event.patrocinadorId)
        : null,
  ...
  async getParticipations(): Promise<Participacao[]> {
    const response =
      await apiClient.get<ParticipacaoComEventoResponse[]>('/participacoes')
    return response.data.map(normalizeParticipation)
  }

  async getParticipationById(id: string): Promise<Participacao> {
    const response = await apiClient.get<ParticipacaoComEventoResponse>(
      `/participacoes/${id}`
    )
  ...
  async getEventParticipations(id: string): Promise<Participacao[]> {
    const response = await apiClient.get<
      | { participacoes?: ParticipacaoComEventoResponse[] }
      | ParticipacaoComEventoResponse[]
    >(`/admin/eventos/${id}/participacoes`)
    const participacoes = Array.isArray(response.data)
      ? response.data
      : response.data.participacoes || []
  ...
  async saveParticipation(participation: Partial<Participacao>): Promise<void> {
    ...
    const nextInscricao =
      Math.max(1000, ...existing.map((item) => Number(item.inscricao) || 0)) + 1
    const payload = {
      id: participation.id ? Number(participation.id) : null,
      inscricao: participation.inscricao
        ? Number(participation.inscricao)
        : nextInscricao,
      tipo: participation.tipo || 'Aluno',
