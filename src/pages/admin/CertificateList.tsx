...
export const CertificateList: React.FC = () => {
  ...
  const latestCerts = certificates
    .map((c) => {
      const enrichedPart = participacoes.find(
        (p) => p.id === c.participacaoId || p.id === c.participacao?.id
      )
      ...
      const event =
        enrichedPart?.evento ||
        (part ? events.find((e) => e.id === part.eventoId) : null)
      return {
        id: c.id,
  ...
  return (
    <div className="admin-page certificates-page">
      ...
      <section
        ...
        aria-labelledby="pending-certificates-title"
      >
        <div className="section-heading flex items-center justify-between mb-4">
          <h2
            id="pending-certificates-title"
      ...
      <section
        className="latest-certificates mt-12"
        aria-labelledby="latest-certificates-title"
      >
        <div className="section-heading mb-4">
          <h2
            id="latest-certificates-title"
            className="text-lg font-bold text-brand-ink-strong"
          >
                ...
                latestCerts.map((cert) => {
                  return (
                    <tr key={cert.id}>
                      ...
                      <td>
                        <strong>{cert.participante}</strong>
                      </td>
                      <td>{cert.evento}</td>
                      <td
                        className="font-mono text-xs max-w-xs truncate"
                        title={cert.assinatura}
                      >
                        {cert.assinatura}
                      </td>
