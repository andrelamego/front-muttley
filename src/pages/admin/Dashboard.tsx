...
export const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .catch((err) =>
        toast.error(err.message || 'Erro ao carregar o painel administrativo.')
      )
  ...
  certs.forEach((c) => {
    ...
    const evt = part
      ? part.evento || events.find((e) => e.id === part.eventoId)
      : null
    if (evt) {
      const eventId = String(evt.id)
      if (!certsByEventMap[eventId]) {
        certsByEventMap[eventId] = {
          total: 0,
          label: evt.tema.substring(0, 15) + '...',
        }
      }
  ...
  const maxCertTotal =
    certData.length > 0 ? Math.max(...certData.map((d) => d.total)) : 1
  ...
  medals.forEach((m) => {
    ...
    const person = part
      ? part.pessoa || people.find((p) => p.id === part.pessoaId)
      : null
    if (person) {
  ...
  const medalData = Object.values(medalsByPersonMap)
    .sort((a, b) => b.total - a.total)
    .slice(0, 5)
  const maxMedalTotal =
    medalData.length > 0 ? Math.max(...medalData.map((d) => d.total)) : 1
  ...
  const taxaPresenca =
    totalInscricoes > 0
      ? Math.round((totalPresentes / totalInscricoes) * 100)
      : 0
  ...
  events.forEach((e) => {
    if (e.disciplinaId) {
      ...
      disciplineEngagement[e.disciplinaId] =
        (disciplineEngagement[e.disciplinaId] || 0) + regCount
    }
  })
    ...
    <div className="dashboard">
      ...
      <section className="events-section mt-8" aria-labelledby="events-title">
        <div className="section-heading flex items-center justify-between mb-4">
          <h2
            id="events-title"
            className="text-xl font-bold text-brand-ink-strong"
          >
            Próximos eventos
          </h2>
          <Link
      ...
      <section className="stats-section mt-8" aria-labelledby="stats-title">
        <h2
          id="stats-title"
          className="text-xl font-bold text-brand-ink-strong mb-4"
        >
        ...
        <div className="bento-grid grid grid-cols-1 md:grid-cols-4 gap-4 mt-4">
          ...
          <article className="bento-card bento-kpi bg-brand-surface border border-brand-line rounded-xl p-5 shadow-sm md:col-span-2 flex flex-col justify-between min-h-[180px]">
            <div>
              ...
              <h3 className="text-base font-bold text-brand-ink-strong mt-0.5 mb-4">
          ...
          <article className="bento-card bg-brand-surface border border-brand-line rounded-xl p-5 shadow-sm md:col-span-1 flex flex-col justify-between min-h-[180px]">
            <div>
              ...
              <h3 className="text-base font-bold text-brand-ink-strong mt-0.5 mb-1">
                Presença Média
              </h3>
            </div>
            <div className="flex flex-col gap-1.5 mt-auto">
              <div className="flex items-baseline gap-1">
                <strong className="text-4xl font-extrabold text-brand-ink-strong">
                  {taxaPresenca}%
                </strong>
          ...
          <article className="bento-card bg-brand-surface border border-brand-line rounded-xl p-5 shadow-sm md:col-span-1 flex flex-col justify-between min-h-[180px]">
            <div>
              ...
              <h3 className="text-base font-bold text-brand-ink-strong mt-0.5 mb-2">
                Disciplina Líder
              </h3>
            </div>
            {topDiscipline ? (
              <div className="mt-auto">
                <strong
                  className="text-sm font-extrabold text-brand-primary block truncate-2-lines leading-tight"
                  title={topDiscipline.nome}
                >
                  {topDiscipline.nome}
                </strong>
          ...
          <article className="bento-card bg-brand-surface border border-brand-line rounded-xl p-5 shadow-sm md:col-span-2 flex flex-col min-h-[280px]">
            <div>
              ...
              <h3 className="text-base font-bold text-brand-ink-strong mt-0.5 mb-3">
                Diplomas por Evento
              </h3>
            </div>
              ...
              <div className="bar-chart vertical flex-grow grid grid-flow-col gap-3 p-3 bg-brand-surface-soft rounded-lg items-end justify-start overflow-x-auto border border-brand-line/30">
                {certsWithPercentage.map((item, idx) => (
                  ...
                  >
                    ...
                    />
                    <strong className="text-[11px] text-brand-ink-strong mt-1">
                      {item.total}
                    </strong>
                    <small
                      className="text-[9px] text-brand-muted truncate w-14 block"
                      title={item.label}
                    >
                      {item.label}
                    </small>
          ...
          <article className="bento-card bg-brand-surface border border-brand-line rounded-xl p-5 shadow-sm md:col-span-2 flex flex-col min-h-[280px]">
            <div>
              ...
              <h3 className="text-base font-bold text-brand-ink-strong mt-0.5 mb-3">
                Destaque: Medalhistas
              </h3>
            </div>
            {medalsWithPercentage.length > 0 ? (
              <ul
                className="ranking-list flex flex-col gap-3 justify-center flex-grow"
                aria-label="Ranking de participantes com mais medalhas"
              >
                {medalsWithPercentage.map((item, idx) => (
                  <li
                    key={idx}
                    className="grid grid-cols-[100px_1fr] gap-2 items-center text-xs"
                  >
                    <span
                      className="truncate text-brand-ink font-semibold"
                      title={item.label}
                    >
                      {item.label.split(' ')[0] +
                        ' ' +
