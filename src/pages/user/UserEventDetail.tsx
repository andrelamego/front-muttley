...
export const UserEventDetail: React.FC = () => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="text-center p-8 bg-brand-surface rounded-xl border border-brand-line shadow-md">
          ...
          <Link
            to="/login"
            className="primary-action mt-4 inline-flex items-center"
          >
            Ir para Login
          </Link>
        </div>
  ...
  const userPart = loggedUser
    ? participacoes.find(
        (p) => p.eventoId === event.id && p.pessoaId === loggedUser.id
      )
    : null
  ...
  const address = local
    ? addresses.find((a) => a.id === local.enderecoId)
    : null
      ...
      <main className="user-event-page max-w-3xl mx-auto px-4">
        ...
        <section className="user-event-hero grid grid-cols-[auto_1fr] gap-6 items-start mt-6">
          <div className="user-event-date-card flex flex-col items-center justify-center p-3 w-20 h-24 bg-brand-surface border border-brand-line rounded-lg shadow-sm">
            ...
            <strong className="text-xs font-bold text-brand-primary uppercase mt-1">
              {monthName}
            </strong>
          </div>
          ...
          <div className="user-event-heading min-w-0">
            <span
              className={`user-event-status ${inscricoesEncerradas ? 'is-closed' : ''} text-xs font-bold uppercase tracking-wide px-3 py-1 rounded-full bg-brand-primary-soft text-brand-primary-strong`}
            >
            ...
            <h1
              id="event-title"
              className="text-3xl font-extrabold text-brand-ink-strong mt-2"
            >
              {event.tema}
            </h1>
          ...
          <article className="user-event-info-card bg-brand-surface p-4 border border-brand-line rounded-lg shadow-sm flex flex-col">
            ...
            {sponsor?.site && (
              <a
                ...
                className="text-xs text-brand-primary hover:underline mt-0.5"
              >
                {sponsor.site}
              </a>
            )}
